...
from models.backbones.vit_encoder import (
    VisionTransformer,
    resize_pos_embed,
)
from models.heads.segmenter_decoder import (
    DecoderLinear,
    MaskTransformer,
)
...
class PIRATSegmenterModel(SegmentationModelWrapper):
    num_classes_attr = "n_cls"
...
def interpolate_pos_encoding(
    ...
    if npatch == N and w == h:
        log.info("Positional encoding not changed.")
        return pos_embed
    log.info(
...
class SegMenter(nn.Module):
    def __init__(self, encoder, decoder, n_cls, backbone, pretrained=""):
        super().__init__()
        self.n_cls = n_cls
        self.patch_size = 16
        ...
        if pretrained is not None:
            ...
            filtered_dict = checkpoint_filter_fn(ckpt, self.encoder)
...
def load_model(model_path):
    ...
    ckpt = {k.replace("image_encoder.", ""): v for k, v in checkpoint.items()}
