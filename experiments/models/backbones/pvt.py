...
class Block(nn.Module):
    def __init__(self, dim, head, sr_ratio=1, mlp_ratio=4, dpr=0.0):
        super().__init__()
...
class PVTv2(nn.Module):
    def __init__(self, model_name: str = "B1") -> None:
        super().__init__()
        assert model_name in pvtv2_settings.keys(), (
            f"PVTv2 model name should be in {list(pvtv2_settings.keys())}"
        )
        ...
        self.block1 = nn.ModuleList(
            [Block(embed_dims[0], 1, 8, 8, dpr[cur + i]) for i in range(depths[0])]
        )
        self.norm1 = nn.LayerNorm(embed_dims[0])

        cur += depths[0]
        self.block2 = nn.ModuleList(
            [Block(embed_dims[1], 2, 4, 8, dpr[cur + i]) for i in range(depths[1])]
        )
        self.norm2 = nn.LayerNorm(embed_dims[1])

        cur += depths[1]
        self.block3 = nn.ModuleList(
            [Block(embed_dims[2], 5, 2, 4, dpr[cur + i]) for i in range(depths[2])]
        )
        self.norm3 = nn.LayerNorm(embed_dims[2])

        cur += depths[2]
        self.block4 = nn.ModuleList(
            [Block(embed_dims[3], 8, 1, 4, dpr[cur + i]) for i in range(depths[3])]
        )
...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load("checkpoints/backbones/pvtv2/pvt_v2_b1.pth", map_location="cpu"),
        strict=False,
    )
