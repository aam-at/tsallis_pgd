...
from models.backbones.convnext_orig import CONVNEXT_SETTINGS

from .wrapper import SegmentationModelWrapper


class PIRATUperModel(SegmentationModelWrapper):
    num_classes_attr = "decode_head.cls"
...
class ModelOutput(OrderedDict):
    ...
    def to_tuple(self):
        """Convert self to a tuple containing all the attributes/keys that are
        not `None`."""
...
class UperNetFCNHead(nn.Module):
    def __init__(
        self, in_index=2, kernel_size=3, dilation=1, in_channels=384, cls=150
    ) -> None:
        super().__init__()
...
class UperNetForSemanticSegmentation(nn.Module):
    def __init__(self, backbone="ConvNext-T", n_cls=150, pretrained=""):
        ...
        self.decode_head = UperNetHead(
            in_channels=CONVNEXT_SETTINGS[variant][1], cls=n_cls
        )
        self.auxiliary_head = UperNetFCNHead(
            in_channels=CONVNEXT_SETTINGS[variant][-2], cls=n_cls
        )
    ...
    def forward(
        ...
        lbl: torch.Tensor | None = None,
    ):
        features = self.backbone(input)

        logits = self.decode_head(features)
        logits = nn.functional.interpolate(
            logits, size=input.shape[2:], mode="bilinear", align_corners=False
        )
