...
config_name="test_cityscapes"
models=(pspnet_cityscapes_ddcat)
    ...
    for epsilon in "${epsilons[@]}"; do
      for job_id in "${job_ids[@]}"; do
        ...
        commands_to_run+=("$(build_command "$config_name" "$model" "$attack" "$task_name" "$epsilon" 300 true \
          "job_id=$job_id" "data.normalize=false" "data.crop_size=449" "data.info.ignore_index=255")")
      done
    done
  done
done

dispatch_commands "ddcat_cityscapes_results"
